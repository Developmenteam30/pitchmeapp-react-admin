...
const UserBlog = (props) => {
  ...
  useEffect(() => {
    getUserBlog(_id, pageLength, pageLimit, '');
  ...
  const onEditBlog = (user) => {
    setUpdateData(true);
    setEditId(user._id);
    getUserBlogById(user._id).then((res) => {
  ...
  const onFieldKeyPress = (e) => {
    if (e.target.name === 'search') {
      if (e.key === 'Enter') {
        getUserBlog(_id, pageLength, pageLimit, e.target.value);
      }
    }
  };
  ...
  const onSubmitForm = (e) => {
    ...
    if (validationFlag === true) {
      ...
      if (UpdateData === true) {
        updateUserBlog(editId, formData)
          .then((res) => {
            setSaveLoading(false);
  ...
  const ondeleteUserBlog = (user) => {
    swal({
      title: 'Are you sure?',
      text: `Are you sure that you want to delete  ${user.title} ?`,
      icon: 'warning',
      buttons: true,
      dangerMode: true,
    }).then((willDelete) => {
      if (willDelete) {
        deleteUserBlog(user._id)
          .then((result) => {
  ...
  const onPageClick = (page) => {
    getUserBlog(_id, page, pageLimit, '');
  };
  ...
  const { userBlog, userBlogLoading } = user_details;
  let { page } = userBlog;
  page = page - 1;
          ...
          <Card body className='px-0 py-0'>
            <CardBody className='px-3 py-3'>
              ...
              <Table responsive striped className='mt-2 customDataTable'>
                ...
                <tbody>
                  {userBlog && userBlog.docs.length > 0 && !userBlogLoading ? (
                    userBlog.docs.map((user, i) => (
                      <tr key={i}>
                  ...
                  ) : userBlog.docs.length === 0 && !userBlogLoading ? (
                    <tr>
                      <td colSpan='6' className='middle-align text-center'>
                        No Blog found
                      </td>
              ...
              <div className='row float-right'>
                <div className='col-md-12 '>{paginationSection(userBlog)}</div>
              </div>
            </CardBody>
          </Card>
        </Col>
            ...
            <Row>
              <Col xs='12'>
                <FormGroup>
                  ...
                  />
                  {errors.error_title && (
                    <div className='invalid-text '>{errors.error_title}</div>
                  )}
                </FormGroup>
              </Col>
                ...
                <FormGroup row>
                  ...
                  <Col xs='12'>
                    ...
                    {errors.error_description && (
                      <div className='invalid-text mt-2'>
                        {errors.error_description}
                      </div>
                    )}
                  </Col>
